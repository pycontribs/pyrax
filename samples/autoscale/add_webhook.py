...
name = ""
while not name:
    name = six.moves.input("Enter a name for this webhook: ")

metadata = {}
while True:
    key = six.moves.input("Enter a metadata key, or press Enter to end metadata "
            "entry: ")
    if not key:
        break
    val = six.moves.input("Enter the value for the key '%s': " % key)
    metadata[key] = val
