...
## Basic Concepts
Autoscale is a tool that enables you to scale your application by adding or removing servers based on monitoring events, a schedule, or arbitrary webhooks.
...
To see the launch configuration for a group, call the `get_launch_config()` method:
...
**server_name** | yes |  | This is used as the base for the name. The autoscale service prepends a 10-character string to this base to it to make the name of each server unique.
...
**personality** | no |  | Small text files that are created on the new servers. _Personality_ is discussed in the [Rackspace Cloud Servers documenation](http://docs.rackspace.com/servers/api/v2/cs-devguide/content/Server_Personality-d1e2543.html)
**networks** | no |  | The networks to which you want new servers to be attached. See the [Create Servers documentation](http://docs.rackspace.com/servers/api/v2/cs-devguide/content/CreateServers.html) for the required format.
...
**Note**: If you pass any metadata values in this call, it **overwrites** any existing metadata. If you simply wish to update an existing metadata key, or add a new key/value pair, you must call the `update_metadata(new_meta)` method instead. This call preserves your existing key/value pairs, and only updates it with your changes.

### Deleting a Scaling Group
When you wish to remove a scaling group, call its `delete()` method:
...
**Note**: If you pass any metadata values in this call, it **overwrites** any existing metadata. If you simply wish to update an existing metadata key in your launch configuration, or add a new key/value pair, you must call the `update_launch_metadata()` method instead. This call preserves your existing key/value pairs, and only updates with your changes.
...
Here are the parameters for the `add_policy()` call:
...
### Updating a Policy
You may update a policy at any time, passing in any or all of the above parameters to change that value. For example, to change the cooldown to 60 seconds, and the number of servers to remove to 3, the call is:
...
### Deleting a Policy
When you wish to remove a policy, call its `delete()` method:
...
**Note**: If you pass any metadata values in this call, it **overwrites** any existing metadata. If you simply wish to update an existing metadata key, or add a new key/value pair, you must call the `webhook.update_metadata(new_meta)` method instead (or the corresponding `au.update_webhook_metadata(sg, policy, webhook, new_meta)`). This call preserves your existing key/value pairs, and only updates it with your changes.
