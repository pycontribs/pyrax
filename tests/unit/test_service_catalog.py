...
class ServiceCatalogTest(unittest.TestCase):
    ...
    def test_url_for_no_match(self):
        sc = self.service_catalog
        self.assertRaises(
            exc.EndpointNotFound, sc.url_for, service_type="test")

    def test_url_for_ambiguous(self):
        sc = self.service_catalog
        self.assertRaises(
            exc.AmbiguousEndpoints, sc.url_for, service_type="object-store")

    def test_url_for_object_store(self):
        sc = self.service_catalog
        ret = sc.url_for(
            service_type="object-store", attr="region", filter_value="DFW")
