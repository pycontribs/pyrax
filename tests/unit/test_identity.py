...
class IdentityTest(unittest.TestCase):
    ...
    def test_set_credential_file(self):
        ...
        # File doesn't exist
        self.assertRaises(
            exc.FileNotFound, ident.set_credential_file, "doesn't exist")
        # Missing section
        with utils.SelfDeletingTempfile() as tmpname:
            with file(tmpname, "wb") as ff:
                ff.write("user = x\n")
            self.assertRaises(
                exc.InvalidCredentialFile, ident.set_credential_file, tmpname)
        # Incorrect section
        with utils.SelfDeletingTempfile() as tmpname:
            with file(tmpname, "wb") as ff:
    ...
    def test_authenticate_fail_creds(self):
        ...
        urllib2.urlopen = Mock(side_effect=urllib2.HTTPError(
            fakes.example_uri, 401, "Bad", {}, DummyResponse()))
    ...
    def test_authenticate_fail_other(self):
        ...
        urllib2.urlopen = Mock(
            side_effect=urllib2.HTTPError(
                fakes.example_uri, 500, "Bad", {}, DummyResponse()))
        self.assertRaises(exc.AuthenticationFailed, ident.authenticate)
        urllib2.urlopen = savopen
