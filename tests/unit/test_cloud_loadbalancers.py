...
class CloudLoadBalancerTest(unittest.TestCase):
    ...
    def test_assure_parent_succeed(self):
        adopted_node = Node(
            address="fake", port="fake", parent=fakes.FakeLoadBalancer())
        diff = "DIFF"
        adopted_node._diff = Mock(return_value=diff)
        adopted_node.parent.update_node = Mock()
        adopted_node.update()
        adopted_node.parent.update_node.assert_called_once_with(
            adopted_node, diff)
    ...
    def test_client_delete_access_list_items(self):
        ...
        lb.manager.delete_access_list_items.assert_called_once_with(
            lb, fake_ids)
    ...
    def test_client_delete_metadata_for_node(self):
        ...
        lb.manager.delete_metadata.assert_called_once_with(
            lb, node=nd, keys=keys)
    ...
    def test_client_set_session_persistence_bad(self):
        ...
        self.assertRaises(
            exc.InvalidSessionPersistenceType,
            clt.set_session_persistence,
            lb,
            "BAD")
    ...
    def test_mgr_delete_unattached_virtualip(self):
        ...
        vip = fakes.FakeVirtualIP()
        self.assertRaises(
            exc.UnattachedVirtualIP, mgr.delete_virtualip, lb, vip)
    ...
    def test_mgr_delete_access_list_items_invalid(self):
        ...
        mgr.get_access_list = Mock(return_value=acc_ids)
        self.assertRaises(
            exc.AccessListIDNotFound, mgr.delete_access_list_items, lb, ids)
    ...
    def test_mgr_update_ssl_termination_no_config(self):
        ...
        self.assertRaises(
            exc.NoSSLTerminationConfiguration, mgr.update_ssl_termination, lb)
    ...
    def test_mgr_get_metadata(self):
        ...
        mgr.api.method_get = Mock(
            return_value=({}, {"metadata": fake_screwy_meta}))
    ...
    def test_mgr_delete_node_metadata(self):
        ...
        uri = "/loadbalancers/%s/nodes/%s/metadata?id=%s" % (
            lb.id, nd.id, fake_meta_id)
        mgr.api.method_delete.assert_called_once_with(uri)
    ...
    def test_mgr_get_usage_period(self):
        ...
        uri = "/loadbalancers/usage?startTime=%s&endTime=%s" % (
            start_iso, end_iso)
        mgr.api.method_get.assert_called_once_with(uri)
    ...
    def test_vip_bad_ip_version(self):
        self.assertRaises(
            exc.InvalidVirtualIPVersion, VirtualIP, ipVersion="FAKE")
    ...
    def test_client_create_body(self):
        ...
        ret = clt._create_body(
            ...
            timeout=fake_timeout, sessionPersistence=fake_sessionPersistence)
    ...
    def test_bad_node_condition(self):
        ...
        fake_sessionPersistence = True
        self.assertRaises(
            exc.InvalidNodeCondition, clt._create_body, fake_name,
            port=fake_port, protocol=fake_protocol, nodes=fake_nodes,
            ...
            connectionThrottle=fake_connectionThrottle,
            healthMonitor=fake_healthMonitor,
            metadata=fake_metadata, timeout=fake_timeout,
            sessionPersistence=fake_sessionPersistence)
    ...
    def test_missing_lb_parameters(self):
        ...
        fake_sessionPersistence = True
        self.assertRaises(
            exc.MissingLoadBalancerParameters, clt._create_body, fake_name,
            port=fake_port, protocol=fake_protocol, nodes=fake_nodes,
            virtual_ips=fake_virtual_ips, algorithm=fake_algorithm,
            accessList=fake_accessList,
            connectionLogging=fake_connectionLogging,
    ...
    def test_client_get_usage(self):
        ...
        clt.get_usage(lb)
        clt._manager.get_usage.assert_called_once_with(
            loadbalancer=lb, start=None, end=None)

    def test_client_allowed_domains(self):
        clt = self.client
        fake_name = utils.random_name()
        fake_body = {
            "allowedDomains": [{"allowedDomain": {"name": fake_name}}]}
