...
class StorageObject(object):
    """A CloudFiles storage object."""
    def __init__(self, client, container, name=None, total_bytes=None,
                 content_type=None, last_modified=None, etag=None,
                 attdict=None):
        """
        The object can either be initialized with individual params,
        or by passing the dict that is returned by swiftclient.
    ...
    def _read_attdict(self, dct):
        """
        Populate the object attributes using the dict returned
        by swiftclient.
    ...
    def purge(self, email_addresses=[]):
        """
        Purge the object from the CDN network, sending an optional
        email confirmation.

        """
        self.client.purge_cdn_object(
            container=self.container.name,
            name=self.name,
    ...
    def set_metadata(self, metadata, clear=False):
        """
        Set this object's metadata, optionally clearing
        existing metadata.

        """
        self.client.set_object_metadata(
            self.container, self, metadata, clear=clear)

    def remove_metadata_key(self, key):
        """
        Remove the specified key from the storage object's metadata.
