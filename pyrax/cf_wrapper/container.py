...
class Container(object):
    ...
    def get_objects(self, marker=None, limit=None, prefix=None, delimiter=None,
        ...
        """
        objs = self.client.get_container_objects(
            self.name,
            marker=marker,
            limit=limit,
            prefix=prefix,
    ...
    def get_object(self, name):
        ...
        if not ret:
            objs = [
                obj for obj in self.client.get_container_objects(self.name)
                if obj.name == name]
            try:
                ret = objs[0]
            except IndexError:
    ...
    def get_object_names(self, marker=None, limit=None, prefix=None,
        ...
        """
        Return a list of the names of all the objects in this
        container. The same pagination parameters apply as
        in self.get_objects().
    ...
    def store_object(self, obj_name, data, content_type=None, etag=None):
        """
        Create a new object in this container, and populates it with
        the given data.

        """
        return self.client.store_object(
            self, obj_name, data, content_type=content_type, etag=etag)

    def upload_file(self, file_or_path, obj_name=None, content_type=None,
                    etag=None, return_none=False):
        """
        Upload the specified file to this container. If no name is
    ...
    def delete_all_objects(self):
        """Delete all objects from this container."""
    ...
    def delete(self, del_objects=False):
        """
        Delete this Container. If the container contains objects, the
        command will fail unless 'del_objects' is passed as True. In
        that case, each object will be deleted first, and then
        the container.

        """
        return self.client.delete_container(
            self.name, del_objects=del_objects)
    ...
    def fetch_object(self, obj_name, include_meta=False, chunk_size=None):
        ...
        return self.client.fetch_object(
            self, obj_name, include_meta=include_meta, chunk_size=chunk_size)
    ...
    def set_web_index_page(self, page):
        """
        Set the header indicating the index page for this container
    ...
    def set_web_error_page(self, page):
        """
        Set the header indicating the error page for this container
        when creating a static website.
    ...
    def make_private(self):
        """
        Disables CDN access to this container. It may still appear
        public until its TTL expires.
