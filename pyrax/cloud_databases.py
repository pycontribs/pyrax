...
class CloudDatabaseInstance(BaseResource):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._user_manager = BaseManager(
            self.manager.api,
    ...
    def list_databases(self):
        """Return a list of the names of all databases for this instance."""
        return self._database_manager.list()

    def list_users(self):
        """Return a list of the names of all users for this instance."""
        return self._user_manager.list()

    def get_database(self, name):
        """
        Find the database in this instance with the specified name, and
        return a CloudDatabaseDatabase object. If no match is found, a
        NoSuchDatabase exception is raised.

        """
        try:
            return [
                db for db in self.list_databases() if db.name == name][0]
        except IndexError:
            raise exc.NoSuchDatabase(
                "No database by the name '%s' exists." % name)

    def get_user(self, name):
        """
        Find the user in this instance with the specified name, and
        return a CloudDatabaseUser object. If no match is found, a
        NoSuchDatabaseUser exception is raised.

        """
        try:
            return [
                user for user in self.list_users() if user.name == name][0]
        except IndexError:
            raise exc.NoSuchDatabaseUser(
                "No user by the name '%s' exists." % name)

    def create_database(self, name, character_set=None, collate=None):
        """
        Create a database with the specified name. If a database with
        ...
        self._database_manager.create(
            name=name,
            character_set=character_set,
            collate=collate,
            return_none=True)

        # Since the API doesn't return the info for creating the
        # database object, we have to do it manually.
        return self._database_manager.find(name=name)

    def create_user(self, name, password, database_names):
        """
        Create a user with the specified name and password, and gives
        that user access to the specified database(s).

        If a user with that name already exists, a BadRequest (400)
        ...
        # Note that passing in non-None values is required for the
        # create_body method to distinguish between this and the
        # request to create and instance.
        self._user_manager.create(
            name=name,
            password=password,
            database_names=database_names,
            return_none=True)

        # Since the API doesn't return the info for creating the user
        # object, we have to do it manually.
        return self._user_manager.find(name=name)

    def _get_name(self, name_or_obj):
        """
        For convenience, many methods accept either an object or the
        name of the object as a parameter, but need the name to send to
        the API. This method handles that conversion.
    ...
    def delete_database(self, name_or_obj):
        """
        Delete the specified database. If no database by that name
    ...
    def delete_user(self, name_or_obj):
        """
        Deletes the specified user. If no user by that name exists, no
        exception will be raised; instead, nothing at all is done.
    ...
    def enable_root_user(self):
        """
        Enable login from any host for the root user and provides the
    ...
    def root_user_status(self):
        """
        Return True or False, depending on whether the root user for
        this instance has been enabled.
...
class CloudDatabaseDatabase(BaseResource):
    """
    This class represents a database on a CloudDatabaseInstance. It is
    not a true cloud entity, but a convenience object for dealing with
    databases on instances.
...
class CloudDatabaseUser(BaseResource):
    ...
    a true cloud entity, but a convenience object for dealing with
    users for instances.
...
class CloudDatabaseFlavor(BaseResource):
    """
    This class represents the available instance configurations, or
    'flavors', which you use to define the memory and CPU size of your
    instance. These objects are read-only.
...
class CloudDatabaseClient(BaseClient):
    ...
    def create_database(self, instance, name, character_set=None,
                        collate=None):
        """
        Create a database with the specified name on the
        given instance.

        """
        return instance.create_database(
            name, character_set=character_set, collate=collate)

    @assure_instance
    def get_database(self, instance, name):
        """
        Find the database in the given instance with the specified
        name, and returns a CloudDatabaseDatabase object. If no match
        is found, a NoSuchDatabase exception is raised.
    ...
    def delete_database(self, instance, name):
        """Delete the database by name on the given instance."""
    ...
    def get_user(self, instance, name):
        """
        Find the user in the given instance with the specified name,
        and returns a CloudDatabaseUser object. If no match is found, a
    ...
    def delete_user(self, instance, name):
        """Delete the user by name on the given instance."""
    ...
    def root_user_status(self, instance):
        """Return True if the given instance is root-enabled."""
    ...
    def get_flavor(self, flavor_id):
        """Return a specific Flavor object by ID."""
    ...
    def _get_flavor_ref(self, flavor):
        ...
        if flavor_obj is None:
            ...
            try:
                flavor_obj = [
                    flav for flav in flavors if flav.name == flavor][0]
            except IndexError:
                # No such name; try matching RAM
                try:
                    flavor_obj = [
                        flav for flav in flavors if flav.ram == flavor][0]
                except IndexError:
                    raise exc.FlavorNotFound(
                        "Could not determine flavor from '%s'." % flavor)
        # OK, we have a Flavor object. Get the href
        href = [
            link["href"] for link in flavor_obj.links
            if link["rel"] == "self"][0]
        return href

    def _create_body(self, name, flavor=None, volume=None, databases=None,
                     users=None, character_set=None, collate=None,
                     password=None, database_names=None):
        """
