...
class BaseResource(object):
    ...
    def human_id(self):
        """
        Subclasses may override this to provide a pretty ID which can
    ...
    def _add_details(self, info):
        """
        Take the dict returned by the API call and set the
    ...
    def __getattr__(self, key):
        ...
        are initially returned. The first time any of the other
        attributes are referenced, a GET is made to get the full
    ...
    def __repr__(self):
        reprkeys = sorted(
            ...
            if (key[0] != "_") and (
                key not in ("manager", "created", "updated")))
        info = ", ".join(
    ...
    def __eq__(self, other):
        """
        Two resource objects that represent the same entity in the
        cloud should be considered equal if they have the same ID. If
        they don't have IDs, but their attribute info matches, they
        are equal.
    ...
    def reload(self):
        """
        Since resource objects are essentially snapshots of the entity
        they represent at the time they are created, they do not update
        as the entity updates. For example, the 'status' attribute can
        change, but the instance's value for 'status' will not. This
        method will refresh the instance with the current state of the
        underlying entity.
